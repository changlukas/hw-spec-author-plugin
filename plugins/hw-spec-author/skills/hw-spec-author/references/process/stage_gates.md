...
- [ ] (id: D1.cross.delta_from_plan) Every clause or mechanism in the spec that was **not** in the user's original written plan has been enumerated and either reverted or confirmed per-item. The plan artifact is a written `PLAN.md` or the `/spec-init` interview transcript — **never** a free-form "no delta" assertion. Scope: sections changed since the previous gate. **Acceptance (mechanically checkable):** a `PLAN_DELTA.md` at the spec root containing **either** a per-item delta confirmation log, **or** — when no written plan artifact exists — the exact line `Delta-from-plan: N/A — <reason>`. `/spec-gate` checks that `PLAN_DELTA.md` exists and matches one of these two forms; absence of the file is a fail, never a silent pass. Guards against the over-design failure mode where unrequested mechanisms accrete across editing rounds.
